...
import { Search, Filter, BookOpen, Layers, Bookmark, CheckCircle, Clock, AlertCircle, X, ShieldAlert, Hash } from 'lucide-react';
  ...
  const filteredBooks = useMemo(() => {
    return books.filter((b) => {
      const matchSearch =
        ...
        b.book_number.includes(searchQuery);

      const matchShelf = selectedShelf === 'all' || b.shelf.toString() === selectedShelf;

      // Phase 7: Manual input filter for shelf row (row_number)
      const normalizedRow = rowQuery.trim().replace(/[۰-۹]/g, (d) => '۰۱۲۳۴۵۶۷۸۹'.indexOf(d).toString());
      const matchRow =
        !normalizedRow ||
        b.row_number.toString() === normalizedRow ||
        b.row_number.toString().includes(normalizedRow);

      const matchSubject = selectedSubject === 'all' || b.subject === selectedSubject;
      const matchAvailability =
        selectedAvailability === 'all' || b.availability_status === selectedAvailability;
  ...
  return (
    <div className="py-12 bg-[#042f2e] min-h-screen text-right">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Header - Phase 6: Strictly 'همه کتاب‌ها' without the word 'کاتالوگ' */}
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-8 border-b border-[#0d9488]/30">
          <div>
            <h1 className="text-2xl sm:text-3xl lg:text-4xl font-black text-white hover-hop">
              همه کتاب‌ها
            </h1>
            <p className="mt-2 text-sm text-[#99f6e4]">
              جستجو و رزرو میان بیش از هفت هزار جلد کتاب در ۱۶ قفسه تخصصی
            </p>
          </div>
        ...
        <div className="mt-8 p-6 rounded-3xl bg-[#073834]/80 border border-[#0d9488]/40 shadow-xl backdrop-blur-sm space-y-4">
          ...
          <div className="relative">
            ...
            <input
              ...
              placeholder="جستجو با عنوان کتاب، نام نویسنده، ناشر یا شماره ثبت کتاب..."
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              >
                ...
                {Array.from({ length: 16 }, (_, i) => i + 1).map((num) => (
                  <option key={num} value={num.toString()}>
                    قفسه شماره {toPersianDigits(num)}
                  </option>
                ))}
              </select>
            </div>

            {/* Shelf Row Filter - Stage 7: Manual Text/Number Input */}
            <div>
              <label className="block text-xs font-bold text-[#99f6e4] mb-1.5 flex items-center gap-1.5">
              ...
              <div className="relative">
                <input
                  ...
                  placeholder="شماره ردیف (مثلاً 217 یا 2)..."
            ...
            <div>
              ...
              <select
                ...
                className="w-full px-3 py-2.5 rounded-xl bg-[#042f2e] border border-[#0d9488]/40 text-white text-xs font-semibold focus:outline-none focus:ring-2 focus:ring-[#84cc16]"
              >
                <option value="all">همه موضوعات ({toPersianDigits(33)} موضوع)</option>
          ...
          <div className="relative w-full max-w-2xl bg-[#042f2e] border-2 border-[#0d9488]/60 rounded-3xl shadow-2xl p-6 sm:p-8 text-right overflow-y-auto max-h-[90vh]">
            ...
            <div className="flex flex-col sm:flex-row gap-6 items-start">
              ...
              <div className="flex-1 space-y-4">
                ...
                {/* Description */}
                <div>
                  <h4 className="text-xs font-bold text-[#99f6e4] mb-1">درباره کتاب:</h4>
