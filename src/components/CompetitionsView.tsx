        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {displayCompetitions.map((comp) => {
            ...
            return (
              <div
                key={comp.id}
                className="bg-[#073834]/80 rounded-3xl p-6 sm:p-8 border border-[#0d9488]/40 shadow-xl flex flex-col justify-between hover:border-[#84cc16]/50 transition-all group overflow-hidden relative"
              >
                {/* Poster / Header Image if available */}
                {comp.poster_url && (
                  <div className="w-full h-48 -mt-6 -mx-6 mb-6 overflow-hidden rounded-t-3xl relative">
                    <img
                      src={comp.poster_url}
                      alt={comp.title}
